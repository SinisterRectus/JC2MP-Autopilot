...
function ApproachPath:Search()
	...
	if Autopilot.vehicle and self.timer:GetMilliseconds() > self.delay and Game:GetState() == GUIState.Game then
		...
		for airport,runways in pairs(Autopilot.airports) do
			for runway in pairs(runways) do
			
				local near_marker = Autopilot.airports[airport][runway].near_marker
				local far_marker = Autopilot.airports[airport][runway].far_marker
				local distance = Vector3.Distance(position, near_marker)
					
				if distance < Autopilot.airports[airport][runway].glide_length and distance < nearest_marker_distance then
				
					local dy = near_marker.y - position.y
					
					local runway_cone_angle = Autopilot.airports[airport][runway].cone_angle
					local pitch_to_plane = math.deg(math.asin(-dy / distance))
					local pitch_from_runway = Autopilot.airports[airport][runway].glide_pitch
					...
					if math.abs(pitch_difference1) < 0.5 * runway_cone_angle then
						...
						if math.abs(heading_difference1) < 0.5 * runway_cone_angle then
							...
							local plane_cone_angle = Autopilot.planes[Autopilot.model].cone_angle
		...
		if nearest_marker then
			self.approach = {}
			self.approach.near_marker = Autopilot.airports[airport_name][runway_name].near_marker
			self.approach.far_marker = Autopilot.airports[airport_name][runway_name].far_marker
			self.approach.angle = Angle(math.rad(Autopilot:HeadingToYaw(runway_direction)), math.rad(Autopilot.airports[airport_name][runway_name].glide_pitch), 0)
			self.approach.sweep_yaw = Angle(math.rad(Autopilot.airports[airport_name][runway_name].cone_angle / 2), 0, 0)
			self.approach.glide_length = Autopilot.airports[airport_name][runway_name].glide_length
		else
			self.approach = nil
		end
