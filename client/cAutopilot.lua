...
function Autopilot:InitGUI()
	...
	gui.window:SetTitle("Autopilot Panel")
	gui.window:SetVisible(false)
	gui.window:SetClosable(false)
	...
	gui.st.ap.gain.label:SetText("Gain")

	gui.st.ap.input.label = Label.Create(gui.st.window)
	gui.st.ap.input.label:SetText("Input")
...
function Autopilot:DrawApproach() -- Subscribed to GameRender
	if config[8].on and self.draw_approach then
		local approach = self.approach
		if approach then
			Render:DrawLine(approach.near_marker, approach.near_marker + approach.angle * Vector3.Forward * approach.glide_length, self.colors[1])
